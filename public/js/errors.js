...
var error_delete_space = $('#delete_space_failed').html();
    ...
    if(error_create_organization = "You are not authorized to create organizations!")
    {
        ...
        $('.btn').click(function(){$("#create_organization").css("display", "none");$('#screen').css("display", "none");$('body').css({"overflow":"auto"}); $('#create_organization_failed').html('');});
        $('#org_name').focus();
    }

    if(error_delete_organization = "You are not authorized to create organizations!")
    {
        $('#screen').css({	"display": "block", opacity: 0.7, "width": "10000px", "height": "10000px"});
        $('body').css({"overflow":"hidden"});
        $('#create_space').fadeIn(600);
        $('.close').click(function(){$("#create_space").css("display", "none");$('#screen').css("display", "none");$('body').css({"overflow":"auto"}); $('#delete_organization_failed').html('');});
        $('.btn').click(function(){$("#create_space").css("display", "none");$('#screen').css("display", "none");$('body').css({"overflow":"auto"}); $('#delete_organization_failed').html('');});
        $('#space_name').focus();
    }
});