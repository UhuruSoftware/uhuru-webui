...
module Uhuru::Webui
  class Webui < Sinatra::Base
    ...
    get '/create_space' do
      ...
      erb :organization, {:locals => {:create_space_dialog => true}, :layout => :layout_user}
      #:card_type => params[:card_type], :card_masked_number => params[:card_masked_number], :all_users => params[:all_users], :spaces_list => params[:spaces_list], :spaces_count => params[:spaces_count], :members_count => params[:members_count], :owners_list => params[:owners_list], :billings_list => params[:billings_list], :auditors_list => params[:auditors_list],
    end
